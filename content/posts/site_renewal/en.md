...
We have recently updated this website, focusing primarily on its internal
structure.

Rather than changing the user-facing experience, this renewal targets the
foundation of how the site is designed and operated.
...
SSG provides a simple and robust architecture, but it requires everything at
build time.
...
the site has been restructured as an application running on the server, rather
than a collection of static files.
...
Since its introduction, Deno has often been discussed as an alternative to
Node.js, but it has not been widely adopted for production.
...
RSC is a technology that can be integrated without requiring a specific
framework,

and demonstrating this point through actual operational examples is one
...
Previously, both source code and content were managed within the same
repository.

While this works at a small scale, several issues became more apparent as
content increased:
