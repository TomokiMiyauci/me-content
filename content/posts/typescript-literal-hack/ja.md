...
これは
[chakra-ui の Union types](https://github.com/chakra-ui/chakra-ui/blob/baa6fee280530137548f961db5cb308100bc3920/packages/styled-system/src/utils/types.ts#L14)
見て知ったわけですが、本家のコードは次のようになっています。
...
`{}` と交差させることで、 `string`
型が**文字列リテラル型の派生元ではないなにか**になるようです。
...
上の例では、`{}` がない場合と同じようにインテリセンスは適応されますが、 `null`
以外を受け入れます。
...
また、[TypeScript の公式リファレンス:Do's and Don'ts](https://www.typescriptlang.org/docs/handbook/declaration-files/do-s-and-don-ts.html#general-types)
では次のように述べられています。
